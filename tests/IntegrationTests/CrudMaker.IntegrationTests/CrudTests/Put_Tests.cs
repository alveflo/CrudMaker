...
namespace CrudMaker.IntegrationTests.CrudTests
{
    public class Put_Tests
    {
        ...
        [Test]
        public async Task When_updating_data_and_entity_doesnt_exist_Then_http_status_is_not_found()
        {
            ...
            var response = await _client.PutAsync($"api/posts/{dto.Id}", new StringContent(JsonConvert.SerializeObject(dto), Encoding.UTF8, "application/json"));

            Assert.AreEqual(HttpStatusCode.NotFound, response.StatusCode);
        }

        [Test]
        public async Task When_updating_data_Then_data_is_stored_in_database()
        {
            using var scope = _factory.Services.GetRequiredService<IServiceScopeFactory>().CreateScope();
            using var dbContext = scope.ServiceProvider.GetRequiredService<TestDbContext>();
